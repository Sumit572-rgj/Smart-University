        ...
        <div class="content-wrapper">
            ...
            <div class="grid" style="grid-template-columns: 1fr 2fr; gap: 1.5rem;">
                <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200" style="height: fit-content;">
                    ...
                    <form action="<?= BASE_URL ?>virtualclassroom" method="POST">
                        <input type="hidden" name="add_data" value="1">
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Topic</label><input type="text"  name="topic" class="form-input w-full" required></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Schedule</label><input type="datetime-local"  name="schedule" class="form-input w-full" required></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Meeting Link</label><input type="url"  name="meeting_link" class="form-input w-full" required></div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="table-container" style="overflow-x: auto;">
                        <table class="table">
                            <thead><tr><th>User</th><th>Topic</th><th>Schedule</th><th>Meeting Link</th><th>Date</th><th>Action</th></tr></thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($r['topic']) ?></td><td><?= htmlspecialchars($r['schedule']) ?></td><td><?= htmlspecialchars($r['meeting_link']) ?></td>
                                    <td class="text-xs text-gray-500"><?= htmlspecialchars(date('M d, Y', strtotime($r['created_at']))) ?></td>
                                    <td><a href="<?= BASE_URL ?>virtualclassroom?delete=<?= $r['id'] ?>" class="text-red-500 hover:underline">Delete</a></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
