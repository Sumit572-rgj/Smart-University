...
class HostelInventoryController extends Controller {
    public function index() {
        ...
        $db = (new Model())->db;
        $table_name = 'mod_hostelinventory';
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_data'])) {
            $stmt = $db->prepare("INSERT INTO $table_name (user_id, item_name, quantity, condition_status) VALUES (?, ?, ?, ?)");
            $stmt->execute([$user['id'], $_POST['item_name'], $_POST['quantity'], $_POST['condition_status']]);
            $this->redirect('/hostelinventory?success=Record+added');
        }

        if (isset($_GET['delete'])) {
            $stmt = $db->prepare("DELETE FROM $table_name WHERE id = ?");
            $stmt->execute([$_GET['delete']]);
            $this->redirect('/hostelinventory?success=Record+deleted');
        }
        ...
        $this->view('hostelinventory/index', $data);
    }
}
