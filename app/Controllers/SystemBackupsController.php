...
class SystemBackupsController extends Controller {
    public function index() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_data'])) {
            $stmt = $db->prepare("INSERT INTO $table_name (user_id, backup_name, size_mb, status) VALUES (?, ?, ?, ?)");
            $stmt->execute([$user['id'], $_POST['backup_name'], $_POST['size_mb'], $_POST['status']]);
            $this->redirect('/systembackups?success=Record+added');
        }

        if (isset($_GET['delete'])) {
            $stmt = $db->prepare("DELETE FROM $table_name WHERE id = ?");
            $stmt->execute([$_GET['delete']]);
            $this->redirect('/systembackups?success=Record+deleted');
        }
        ...
        $this->view('systembackups/index', $data);
    }
}
