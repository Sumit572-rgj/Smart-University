...
class PayrollManagementController extends Controller {
    public function index() {
        ...
        $db = (new Model())->db;
        $table_name = 'mod_payrollmanagement';
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_data'])) {
            ...
            $this->redirect('/payrollmanagement?success=Record+added');
        }

        if (isset($_GET['delete'])) {
            $stmt = $db->prepare("DELETE FROM $table_name WHERE id = ?");
            $stmt->execute([$_GET['delete']]);
            $this->redirect('/payrollmanagement?success=Record+deleted');
        }
        ...
        $this->view('payrollmanagement/index', $data);
    }
}
