        ...
        <div class="content-wrapper">
            ...
            <div class="grid" style="grid-template-columns: 1fr 2fr; gap: 1.5rem;">
                <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200" style="height: fit-content;">
                    ...
                    <form action="<?= BASE_URL ?>disciplinaryrecords" method="POST">
                        <input type="hidden" name="add_data" value="1">
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Incident</label><input type="text"  name="incident" class="form-input w-full" required></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Action Taken</label><textarea name="action_taken" class="form-input w-full" rows="3" required></textarea></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Incident Date</label><input type="date"  name="incident_date" class="form-input w-full" required></div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="table-container" style="overflow-x: auto;">
                        <table class="table">
                            <thead><tr><th>User</th><th>Incident</th><th>Action Taken</th><th>Incident Date</th><th>Date</th><th>Action</th></tr></thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($r['incident']) ?></td><td><?= htmlspecialchars($r['action_taken']) ?></td><td><?= htmlspecialchars($r['incident_date']) ?></td>
                                    <td class="text-xs text-gray-500"><?= htmlspecialchars(date('M d, Y', strtotime($r['created_at']))) ?></td>
                                    <td><a href="<?= BASE_URL ?>disciplinaryrecords?delete=<?= $r['id'] ?>" class="text-red-500 hover:underline">Delete</a></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
