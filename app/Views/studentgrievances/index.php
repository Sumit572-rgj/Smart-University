        ...
        <div class="content-wrapper">
            ...
            <div class="grid" style="grid-template-columns: 1fr 2fr; gap: 1.5rem;">
                <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200" style="height: fit-content;">
                    ...
                    <form action="<?= BASE_URL ?>studentgrievances" method="POST">
                        <input type="hidden" name="add_data" value="1">
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Category</label><input type="text"  name="category" class="form-input w-full" required></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Complaint</label><textarea name="complaint" class="form-input w-full" rows="3" required></textarea></div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="table-container" style="overflow-x: auto;">
                        <table class="table">
                            <thead><tr><th>User</th><th>Category</th><th>Complaint</th><th>Status</th><th>Date</th><th>Action</th></tr></thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($r['category']) ?></td><td><?= htmlspecialchars($r['complaint']) ?></td><td><?= htmlspecialchars($r['status']) ?></td>
                                    <td class="text-xs text-gray-500"><?= htmlspecialchars(date('M d, Y', strtotime($r['created_at']))) ?></td>
                                    <td><a href="<?= BASE_URL ?>studentgrievances?delete=<?= $r['id'] ?>" class="text-red-500 hover:underline">Delete</a></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
