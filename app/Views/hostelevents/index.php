        ...
        <div class="content-wrapper">
            ...
            <div class="grid" style="grid-template-columns: 1fr 2fr; gap: 1.5rem;">
                <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200" style="height: fit-content;">
                    ...
                    <form action="<?= BASE_URL ?>hostelevents" method="POST">
                        <input type="hidden" name="add_data" value="1">
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Event Name</label><input type="text"  name="event_name" class="form-input w-full" required></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Event Date</label><input type="date"  name="event_date" class="form-input w-full" required></div>
                        <div class="mb-3"><label class="block text-sm font-medium mb-1">Description</label><textarea name="description" class="form-input w-full" rows="3" required></textarea></div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="table-container" style="overflow-x: auto;">
                        <table class="table">
                            <thead><tr><th>User</th><th>Event Name</th><th>Event Date</th><th>Description</th><th>Date</th><th>Action</th></tr></thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($r['event_name']) ?></td><td><?= htmlspecialchars($r['event_date']) ?></td><td><?= htmlspecialchars($r['description']) ?></td>
                                    <td class="text-xs text-gray-500"><?= htmlspecialchars(date('M d, Y', strtotime($r['created_at']))) ?></td>
                                    <td><a href="<?= BASE_URL ?>hostelevents?delete=<?= $r['id'] ?>" class="text-red-500 hover:underline">Delete</a></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
