<?php
$file = __DIR__ . '/app/Views/fee/student.php';
if (file_exists($file)) {
    $content = file_get_contents($file);

    // Replace openPaymentModal with openDemoRazorpay
    $content = preg_replace(
        '/onclick="openPaymentModal\((.*?),\s*(.*?)\)"/',
        'onclick="openDemoRazorpay($1, $2, \'Student Fee\')"',
        $content
    );

    // If there is an existing payment modal in student.php, remove it so we can append our robust one
    $content = preg_replace('/<!-- Payment Modal -->.*?<\/script>/is', '', $content);
...
HTML;
    ...
    echo "Demo Razorpay Modal and Coin Animations added to Student portal.";
}
