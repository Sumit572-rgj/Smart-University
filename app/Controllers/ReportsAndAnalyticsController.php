...
class ReportsAndAnalyticsController extends Controller {
    public function index() {
        ...
        $db = (new Model())->db;
        $table_name = 'mod_reportsandanalytics';
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_data'])) {
            $stmt = $db->prepare("INSERT INTO $table_name (user_id, report_type, summary) VALUES (?, ?, ?)");
            $stmt->execute([$user['id'], $_POST['report_type'], $_POST['summary']]);
            $this->redirect('/reportsandanalytics?success=Record+added');
        }

        if (isset($_GET['delete'])) {
            $stmt = $db->prepare("DELETE FROM $table_name WHERE id = ?");
            $stmt->execute([$_GET['delete']]);
            $this->redirect('/reportsandanalytics?success=Record+deleted');
        }
        ...
        $this->view('reportsandanalytics/index', $data);
    }
}
